...
class MiniGPTBase(BaseModel):
    ...
    def generate(
        ...
        temperature=1,
        do_sample=False,
        stop_words_ids=None,  # auto-detect from tokenizer (LLaMA-2: [2], LLaMA-3: [128009])
    ):
        '''
        ...
        #     )
        answers = []
        # Determine the end-of-sequence token string for the current tokenizer.
        # LLaMA-2 uses </s>; LLaMA-3 uses <|eot_id|> or <|end_of_text|>.
        eos_str = self.llama_tokenizer.decode(
            [self.llama_tokenizer.eos_token_id], skip_special_tokens=False
        ) if self.llama_tokenizer.eos_token_id is not None else "</s>"
        bos_str = self.llama_tokenizer.decode(
            [self.llama_tokenizer.bos_token_id], skip_special_tokens=False
        ) if self.llama_tokenizer.bos_token_id is not None else "<s>"

        for output_token in outputs:
            if output_token[0] == 0:
                output_token = output_token[1:]
            output_texts = self.llama_tokenizer.decode(output_token, skip_special_tokens=True)
            # Strip any trailing end-of-sequence markers.
            for eos_variant in [eos_str, "<|eot_id|>", "<|end_of_text|>", "</s>"]:
                if eos_variant and eos_variant in output_texts:
                    output_texts = output_texts.split(eos_variant)[0]
            # Strip BOS if it leaked through (rare with skip_special_tokens=True).
            if bos_str and bos_str in output_texts:
                output_texts = output_texts.replace(bos_str, "")
            # Extract the assistant's response (everything after [/INST]).
            output_texts = output_texts.split(r'[/INST]')[-1].strip()
            # Also handle LLaMA-3 header format: after "assistant\n\n"
            if "<|end_header_id|>" in output_texts:
                output_texts = output_texts.split("<|end_header_id|>")[-1].strip()
