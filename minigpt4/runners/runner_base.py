...
class RunnerBase:
    ...
    def _save_checkpoint(self, cur_epoch, is_best=False):
        ...
        # P2-17: Auto-cleanup old checkpoints (keep only max_keep_ckpts)
        max_keep = self.config.run_cfg.get("max_keep_ckpts", 0)
        if max_keep > 0 and not is_best:
            ckpt_files = []
            for f in os.listdir(self.output_dir):
                if f.startswith("checkpoint_") and f.endswith(".pth") and f != "checkpoint_best.pth":
                    try:
                        epoch_num = int(f.replace("checkpoint_", "").replace(".pth", ""))
                        ckpt_files.append((epoch_num, os.path.join(self.output_dir, f)))
                    except ValueError:
                        continue
            ckpt_files.sort(key=lambda x: x[0])
            while len(ckpt_files) > max_keep:
                _, oldest_path = ckpt_files.pop(0)
                try:
                    os.remove(oldest_path)
                    logging.info("Removed old checkpoint: {}".format(oldest_path))
                except OSError:
                    pass
